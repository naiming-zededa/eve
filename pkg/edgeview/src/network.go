...
func runNetwork(netw string) {
	...
	for _, opt := range opts {
		...
		if opt == "ping" || opt == "trace" {
			server, _ = runCmd("cat /config/server", false, false)
...
func doAppNet(status, appstr string, isSummary bool) string {
	...
	for _, item := range appStatus.UnderlayNetworkList {
		...
		_, _ = runCmd("ping -c 3 "+ipStr, false, true)
	...
	if err == nil {
		...
		_ = json.Unmarshal([]byte(retStr), &domainS)
...
func getAppNetTable(ipaddr string, niStatus *types.NetworkInstanceStatus) {
	...
	for _, l := range lines[:n-1] {
		...
		fmt.Printf(" ip rule: \n" + l + "\n")
		fmt.Printf("\n for table: " + table + "\n")
		_, _ = runCmd("ip route show table "+table, false, true)
	}
}
...
func getVifStats(vifStr string) {
	...
	_ = json.Unmarshal([]byte(retStr), &ntMetric)
...
func getPortCfg(opt string, isPrint bool) []string {
	...
	_ = json.Unmarshal([]byte(outStr), &portcfg)
...
func getLocalIPs() []string {
	var localIPs []string
	intfStr, err := runIntfCmd()
	if err != nil {
		return  localIPs
	}
	r := strings.Split(intfStr, "\n")
	n := len(r)
	for _, l := range r[:n-1] {
		fields := strings.Fields(l)
		length := len(fields)
		ips := strings.Split(fields[length-1], "/")
		if len(ips) != 2 {
			continue
		}
		localIPs = append(localIPs, ips[0])
	}
	return localIPs
}
...
func getConnectivity() {
	fmt.Printf("  run diag: \n")
	_, _ = runCmd("/opt/zededa/bin/diag -o /dev/stdout", true, true)
	...
	_ = json.Unmarshal([]byte(retStr), &portlist)
...
func runACLs(isRunningACL bool, filter string) {
	...
	for _, tbl := range acltables {
		...
		_, _ = runCmd("iptables "+op+" -t "+tbl, true, true)
	}
}
...
func runRoute() {
	...
	for _, table := range tables {
		printColor("show route in table: "+table, colorCYAN)
		_, _ = runCmd("ip route show table "+table, false, true)
	}
}
...
func getMetricsMap(path string, stats *urlStats, isPrint bool) {
	_, err := runCmd("ls "+path, false, false)
	if err != nil {
		return
	}
...
func getDNS(domain string) {
	...
	printColor(" - nslookup: "+domain, colorCYAN)
	_, _ = runCmd("nslookup "+domain, false, true)
}

func showSockets() {
	printColor(" listening socket ports: ", colorBLUE)
	_, _ = runCmd("ss -tunlp4", true, true)
	printColor(" socket established: ", colorBLUE)
	_, _ = runCmd("ss -t state established", true, true)
}

func showIntf(substring, intfStr string) {
	fmt.Printf("%s\n", intfStr)
	printTitle(" ip -br link:", colorCYAN, false)
...
func runTrace(substring, server string) {
	if substring != "" {
		...
		_, _ = runCmd("traceroute -4 -m 10 -q 2 www.google.com", false, true)
		if server != "" {
			printTitle(" traceroute to "+server, colorCYAN, true)
			_, _ = runCmd("traceroute -4 -m 10 -q 2 "+server, false, true)
		}
	}
}
...
func runPing(intfStr, server string, opt string) {
	if opt != "" {
		if strings.Contains(opt, "/") {
			...
			_, _ = runCmd("ping -c 3 -I "+intf+" "+ipaddr, false, true)
		} else {
			printColor("\n - ping "+opt, colorCYAN)
			_, _ = runCmd("ping -c 3 "+opt, false, true)
		}
		return
	}
	if intfStr == "" {
	...
	for _, intfline := range intfs {
		...
		ipaddr := "8.8.8.8"
		printColor("\n - ping "+ipaddr+" through intf: "+intf[0], colorCYAN)
		...
		printColor("\n - ping to "+server+", source "+ipa.String(), colorCYAN)
		if runOnServer {
			httpsclient(server, ipa)
		} else {
			_, _ = runCmd("curl https://"+server+":/api/v1/edgeDevice/ping", true, true)
		}
		closePipe(true)
	}
}
...
func getProxy(needPrint bool) (string, int, [][]byte) {
	...
	var portcfg types.DevicePortConfig
	_ = json.Unmarshal([]byte(retStr), &portcfg)
...
func runTCPDump(subStr string) {
	...
	printTitle(" tcpdump with: "+cmd, colorGREEN, true)
	retStr, err := runCmd(cmd, false, false)
	if err != nil {
		fmt.Printf("err %v\n", err)
	} else {
...
func runWireless() {
	...
	_, _ = runCmd("iwconfig wlan0", false, true)
...
func runSpeedTest(intf string) {
	...
	_, _ = runCmd("/usr/bin/speedtest"+opt, false, true)
}
