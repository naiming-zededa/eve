...
func (z *zedrouter) checkAndRecreateAppNetworks(niID uuid.UUID) {
	...
	for _, st := range items {
		...
		wasAwaiting := appNetStatus.AwaitNetworkInstance
		...
		} else if wasAwaiting && !appNetStatus.HasError() &&
			!appNetStatus.AwaitNetworkInstance && appNetStatus.Activated {
			// Apply the config change that handleAppNetworkModify dropped while
			// the network instance was not ready yet.
			z.handleAppNetworkModify(nil, appNetConfig.Key(), *appNetConfig, *appNetConfig)
		}
