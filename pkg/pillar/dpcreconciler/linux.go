...
type LinuxDpcReconciler struct {
	...
	HVTypeKube           bool
	encCellularPasswords map[string]*encryptedPassword // key: port logical label
}
...
func (r *LinuxDpcReconciler) init() (startWatcher func()) {
	...
	r.encCellularPasswords = make(map[string]*encryptedPassword)
...
func (r *LinuxDpcReconciler) getIntendedWwanConfig(dpc types.DevicePortConfig,
	...
	if level, err := logrus.ParseLevel(logLevel); err == nil {
		verboseLogging = level >= logrus.DebugLevel
	} else {
	...
	// The ones that are still required will have their status restored,
	// while the remaining ones will be garbage collected at the end.
	for _, encPassword := range r.encCellularPasswords {
		encPassword.used = false
	}
	...
	for _, port := range dpc.Ports {
		...
		if decBlock.CellularNetUsername != "" {
			encPassword, err = r.encryptCellularPassword(port.Logicallabel,
				decBlock.CellularNetPassword)
			if err != nil {
				r.Log.Error(err)
				r.Log.Warnf(
		...
		network := types.WwanNetworkConfig{
			...
			Username:          decBlock.CellularNetUsername,
	...
	// GC no longer used passwords.
	for portLL, encPassword := range r.encCellularPasswords {
		if !encPassword.used {
			delete(r.encCellularPasswords, portLL)
		}
	}
...
func (r *LinuxDpcReconciler) encryptCellularPassword(portLL, password string) (
	...
	if encPassword != nil && encPassword.cleartext == password {
		// Encryption algorithm uses random salt and IV meaning that it will return
		// different ciphertext on each run even for the same password.
		...
		encPassword.used = true
		return encPassword.ciphertext, nil
	}
